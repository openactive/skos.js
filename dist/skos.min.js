...
function ConceptScheme(a,b){if(Array.isArray(a)){if("undefined"===typeof b)throw Error("ID must be supplied with Concept array");this.scheme={type:"ConceptScheme",id:b,concept:a}}else if(a.concept&&a.id&&"ConceptScheme"===a.type)this.scheme=a;else throw Error("Invalid scheme supplied to ConceptScheme");a=[];b={};for(var d={},e=[],c,g=0;g<this.scheme.concept.length;g++){c=new Concept(this.scheme.concept[g]);d[c.id]=c;b[c.prefLabel]=c;e.push(c);if(c.altLabel&&Array.isArray(c.altLabel))for(var f=0;f<
c.altLabel.length;f++)b[c.altLabel[f]]=c;this.scheme.concept[g].topConceptOf===this.scheme.id&&a.push(c)}for(g=0;g<e.length;g++){c=e[g];f=c._originalConcept.broader||c._originalConcept.broaderTransitive||[];Array.isArray(f)||(f=[f]);for(var h=0;h<f.length;h++){var k=f[h];if(d[k])c._broaderConcepts.push(d[k]);else throw Error("Invalid scheme supplied to ConceptScheme: referenced Concept "+k+"not found in scheme");}if(c._originalConcept.related&&Array.isArray(c._originalConcept.related))for(f=0;f<c._originalConcept.related.length;f++)if(h=
c._originalConcept.related[f],d[h])c._relatedConcepts.push(d[h]);else throw Error("Invalid scheme supplied to ConceptScheme: referenced Concept "+h+"not found in scheme");}for(g=0;g<e.length;g++)for(c=e[g],f=0;f<c._broaderConcepts.length;f++)c._broaderConcepts[f]._narrowerConcepts.push(c);this.topConcepts=a.sort(Concept.compare);this.index=d;this.labelIndex=b}
ConceptScheme.prototype.getConceptByID=function(a){if(this.index[a])return this.index[a];a="oa:"===a.substring(0,3)?a.replace("oa:","https://openactive.io/"):a;return this.index[a]?this.index[a]:null};ConceptScheme.prototype.getConceptByLabel=function(a){return this.labelIndex[a]||null};ConceptScheme.prototype.getAllConcepts=function(){var a=this.index;return(Object.values?Object.values(a):Object.keys(a).map(function(b){return a[b]})).sort(Concept.compare)};
ConceptScheme.prototype.getAllConceptsByID=function(){return this.index};ConceptScheme.prototype.getAllConceptsByLabel=function(){return this.labelIndex};ConceptScheme.prototype.getTopConcepts=function(){return this.topConcepts};ConceptScheme.prototype.getJSON=function(){return this.scheme};
ConceptScheme.prototype.toString=function(){function a(b,d){return b.getNarrower().reduce(function(b,c){return b.concat(a(c,d+2))},[Array(d).join(" ")+"- "+b.toString()])}return this.topConcepts.reduce(function(b,d){return b.concat(a(d,1))},[]).join("\n")};
